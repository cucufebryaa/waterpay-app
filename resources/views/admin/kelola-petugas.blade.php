...
<div class="page-header">
    ...
    <button type="button" class="btn-add" data-bs-toggle="modal" data-bs-target="#modalTambahPetugas">
...
<script>
    ...
    function showDetailAlert(petugasData) {
        const detailPetugas = {
            ...
            'No. HP': petugasData.no_hp || '-',
            'ID Petugas': petugasData.id || '-',
            'ID Perusahaan': petugasData.id_company || '-'
        };

        const detailUser = petugasData.user ? {
            'Username': petugasData.user.username || '-',
            'Email': petugasData.user.email || '-',
            'NIK': petugasData.user.nik || '-',
            'ID User': petugasData.user.id || '-'
        } : {
    ...
    function showDeleteAlert(petugasId, petugasName) {
        Swal.fire({
            title: 'Hapus Petugas?',
            html: `Anda yakin ingin menghapus <strong>"${petugasName}"</strong>?<br><small class="text-muted">Tindakan ini tidak dapat dibatalkan!</small>`,
            icon: 'warning',
